...
def is_market_open(now: Optional[datetime] = None, at_all_hours: bool = False) -> tuple[bool, str]:
    """Return ``(open, reason)`` for trading now.

    If ``at_all_hours`` is True, always returns (True, "open").
    """
    if at_all_hours:
        return True, "open"
