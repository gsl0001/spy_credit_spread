...
export function RiskView() {
  ...
  return (
    <div className="page">
      <div className="grid g-3" style={{ marginBottom: 14 }}>
        <Card title="Pre-trade gate" icon="shield" actions={<Badge variant="pos" dot>ACTIVE</Badge>}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Market hours</span><Chip ok={r.market_open}>{r.market_open ? `Open · closes ${r.next_close || '—'}` : 'Closed'}</Chip></div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Buying power</span><Chip ok={m.account.buying_power > 0}>${m.account.buying_power.toLocaleString()}</Chip></div>
        ...
        <Card title="Capacity" icon="target">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <RiskBar label="Concurrent positions" used={r.current_concurrent} limit={r.max_concurrent} unit="" />
            <RiskBar label="Daily loss (of cap)" used={+(r.daily_loss_used_pct / r.daily_loss_limit_pct * 100).toFixed(1)} limit={100} unit="%" />
            <RiskBar label="Buying power used" used={+r.buying_power_used_pct.toFixed(1)} limit={100} unit="%" />
          </div>
        </Card>
        ...
        <Card title="Sizing" icon="sliders">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10, fontSize: 12 }}>
            ...
            <div className="ticket__row"><span className="k">Target % of equity</span><span className="v">{r.target_pct.toFixed(2)}%</span></div>
            <div className="ticket__row"><span className="k">Max trade cap</span><span className="v">${r.max_trade_cap}</span></div>
            <div className="ticket__row"><span className="k">Bid-ask haircut</span><span className="v">{r.haircut_bps} bps</span></div>
            <hr className="sep" style={{ margin: '4px 0' }} />
            <div className="muted" style={{ fontSize: 11 }}>Sizing clamped by excess_liquidity ${m.account.excess_liquidity.toLocaleString()}</div>
          </div>
        </Card>
      </div>
      ...
      <Card title="Daily digest webhook" icon="send" actions={<Btn size="sm" icon="send">Send test</Btn>}>
